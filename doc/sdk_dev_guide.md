...
#### `tera::ScanDescriptor / tera::ResultStream` 遍历

`tera::ScanDescriptor`描述遍历需求，包括行区间、列集合、时间区间、版本、特殊过滤器等。

由`tera::Table Scan()`生效，返回迭代器`tera::ResultStream`。

#### `tera::TableDescriptor / tera::LocalityGroupDescriptor / tera::ColumnFamilyDescriptor`表格描述
...
// RowMutation 增删（保证多列原子操作，支持同步、异步）
