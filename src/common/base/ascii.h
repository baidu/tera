...
struct Ascii {
 ...
 public:
  /** 判断是不是有效的 ASCII 码 */
  static bool IsValid(char c) { return (c & 0x80) == 0; }

  static inline bool IsLower(char c) { return CharIncludeAnyTypeMask(c, kLower); }

  static inline bool IsUpper(char c) { return CharIncludeAnyTypeMask(c, kUpper); }

  /** 判断是否为字母 */
  static bool IsAlpha(char c) { return CharIncludeAnyTypeMask(c, kUpper | kLower); }

  /** 判断是否为数字 */
  static bool IsDigit(char c) { return CharIncludeAnyTypeMask(c, kDigit); }

  /** 判断是否为英文或数字  */
  static bool IsAlphaNumber(char c) { return CharIncludeAnyTypeMask(c, kUpper | kLower | kDigit); }

  /** 判断是否为空白字符。空格,'\t', ' ' 算作空白字符*/
  static bool IsBlank(char c) { return CharIncludeAnyTypeMask(c, kBlank); }

  /** 判断是否为间隔字符。*/
  static inline bool IsSpace(char c) { return CharIncludeAnyTypeMask(c, kSpace); }

  /** 判断是否为控制字符。*/
  static bool IsControl(char c) { return CharIncludeAnyTypeMask(c, kControl); }

  /** 判断是否为标点符号字符。*/
  static inline bool IsPunct(char c) { return CharIncludeAnyTypeMask(c, kPunct); }

  /** 判断是否为十六进制数字字符。*/
  static inline bool IsHexDigit(char c) { return CharIncludeAnyTypeMask(c, kHexDigit); }

  /** 判断是否为可见字符。*/
  static inline bool IsGraph(char c) { return CharIncludeAnyTypeMask(c, kGraph); }

  /** 判断是否为可打印字符。*/
  static inline bool IsPrint(char c) { return CharIncludeAnyTypeMask(c, kPrint); }
 ...
 private:
  ...
  static bool CharIncludeAnyTypeMask(char c, int mask) { return (GetCharTypeMask(c) & mask) != 0; }
