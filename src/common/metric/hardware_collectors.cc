...
DEFINE_int64(tera_hardware_collect_period_second, 5,
             "hardware metrics checking period (in second)");

namespace tera {
using FileWrapper = std::function<void(FILE*)>;
static FileWrapper file_wrapper = [](FILE* f) {
  if (f) fclose(f);
};
...
#else
  std::unique_ptr<FILE, FileWrapper> fp(fopen("/proc/stat", "r"), file_wrapper);
  if (!fp) {
...
static bool ProcessCpuTick(const std::string& stat_path, int64_t* tick) {
  ...
  std::unique_ptr<FILE, FileWrapper> fp(fopen(stat_path.c_str(), "r"), file_wrapper);
  if (!fp) {
  ...
  long long utime = 0;
  long long stime = 0;
  if (fscanf(fp.get(), "%*s %*s %*s %*s %*s %*s %*s %*s %*s %*s %*s %*s %*s %lld %lld", &utime,
             &stime) < 2) {
...
int64_t CpuUsageCollector::CheckCpuUsage(int64_t cur_ts, bool is_irix_on) {
  ...
  float usage_percentage =
      static_cast<float>(new_tick_total - last_tick_total_) * 100.0f / interval_total_ticks;
...
int64_t MemUsageCollector::CheckMemUsage(int64_t cur_ts) {
  std::unique_ptr<FILE, FileWrapper> stat_file(fopen(stat_path_.c_str(), "r"), file_wrapper);
  if (!stat_file) {
  ...
  int64_t mem_pages = 0;
  fscanf(stat_file.get(), "%*d %ld", &mem_pages);
...
NetUsageCollector::NetUsageCollector(NetUsageType n_type) : net_usage_type_(n_type) {}
...
int64_t NetUsageCollector::Collect() {
  int64_t cur_ts = get_millis();
  int64_t collect_period_ms = FLAGS_tera_hardware_collect_period_second * 1000;
  if (collect_period_ms > 0 && cur_ts < net_info_checker_.last_check_time_ms_ + collect_period_ms) {
    return net_usage_type_ == RECEIVE ? net_info_checker_.net_rx_usage_
                                      : net_info_checker_.net_tx_usage_;
  } else {
    int64_t value = 0;
...
bool NetUsageCollector::NetInfoChecker::GetCurrentTotal(int64_t* rx_total, int64_t* tx_total) {
  ...
  int ret = fseek(stat_file.get(), 327, SEEK_SET);
  CHECK_EQ(ret, 0);
  for (int i = 0; i < 10; i++) {
    while (':' != fgetc(stat_file.get()))
      ;
    ret = fscanf(stat_file.get(), "%ld%*d%*d%*d%*d%*d%*d%*d%ld", rx_total, tx_total);
...
bool NetUsageCollector::NetInfoChecker::CheckNetUsage(int64_t cur_ts, int64_t* rx_usage,
                                                      int64_t* tx_usage) {
