...
DEFINE_bool(tera_prometheus_subscriber_dump_log, false, "Whether to dump prometheus subscriber log");
...
ReportItem PrometheusSubscriber::Collect() {
...

    {
        ...
        if (tera_queue_ptr_->empty()) {
            LOG_IF(WARNING, FLAGS_tera_prometheus_subscriber_dump_log) << "[PROMETHEUS SUBSCRIBER] Empty Tera Queue";
            return ret;
        }
...
void PrometheusSubscriber::Append(int64_t time_stamp, int64_t current_value) {
    ...
    LOG_IF(WARNING, FLAGS_tera_prometheus_subscriber_dump_log)
        << "[PROMETHEUS APPEND] " << metric_id_.GetName()
        << "\tValue: "  << current_value
