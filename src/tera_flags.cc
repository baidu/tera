...
DEFINE_bool(tera_tabletnode_flash_block_cache_enabled, false, "enable flash block cache mechasism");
...
// those flags prefixed with "tera_master" are shared by several modules which cannot be move to
// master/master_flags, so they are kept here for compatibility until all flags are moved to their own dir
...
DEFINE_int32(tera_timeoracle_refresh_lease_second, 10, "(s) timeoracle refresh lease before this seconds");
