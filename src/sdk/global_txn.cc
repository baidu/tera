...
DECLARE_int32(tera_gtxn_commit_timeout_ms);
...
Transaction* GlobalTxn::NewGlobalTxn(std::shared_ptr<tera::ClientImpl> client_impl,
                                     common::ThreadPool* thread_pool,
                                     sdk::ClusterFinder* tso_cluster) {
  if (client_impl && thread_pool != NULL) {
    std::shared_ptr<GlobalTxn> global_txn_shared_ptr(
        new GlobalTxn(client_impl, thread_pool, tso_cluster));
    return new tera::TransactionWrapper<GlobalTxn>(global_txn_shared_ptr);
  }
  LOG(ERROR) << "client_impl or tso_cluster is NULL";
  return NULL;
}

GlobalTxn::GlobalTxn(std::shared_ptr<tera::ClientImpl> client_impl, common::ThreadPool* thread_pool,
                     sdk::ClusterFinder* tso_cluster)
    : gtxn_internal_(new GlobalTxnInternal(client_impl)),
      ...
      commit_timeout_ms_(FLAGS_tera_gtxn_commit_timeout_ms),
      ttl_timestamp_ms_(FLAGS_tera_gtxn_timeout_ms + get_millis()),
...
void GlobalTxn::SetTimeout(int64_t timeout_ms) { commit_timeout_ms_ = timeout_ms; }

void GlobalTxn::SetReaderStatusAndRunCallback(RowReaderImpl* reader_impl, ErrorCode* status) {
  gtxn_read_cnt.Inc();
...
ErrorCode GlobalTxn::Get(RowReader* row_reader) {
  ...
  if (has_commited_.load()) {
    std::string reason = "get failed, txn has commited @ [" + std::to_string(start_ts_) + "," +
...
void GlobalTxn::AsyncGetCell(Cell* cell, RowReaderImpl* user_reader_impl,
                             InternalReaderContext* ctx) {
  VLOG(12) << "[gtxn][get][" << start_ts_ << "] "
           << gtxn_internal_->DebugString(
                  *cell, "TryGet times(" + std::to_string(ctx->cell_map[cell]) + ")");
  ...
  reader->AddColumn(cell->ColFamily(), cell->DataName());
  ...
  reader->SetCallBack([](RowReader* r) {
    ...
    gtxn->thread_pool_->AddTask(
        std::bind(&GlobalTxn::DoGetCellReaderCallback, gtxn, static_cast<RowReaderImpl*>(r)));
  });
...
void GlobalTxn::DoGetCellReaderCallback(RowReader* reader) {
  ...
  if (gtxn_internal_->IsLockedByOthers(row, *cell)) {
    ...
    if (status.GetType() == ErrorCode::kOK) {
      ...
      AsyncGetCell(cell, static_cast<RowReaderImpl*>(internal_reader_ctx->user_reader),
                   internal_reader_ctx);
      delete reader;
      delete ctx;
      return;
...
void GlobalTxn::MergeCellToRow(RowReader* internal_reader, const ErrorCode& status) {
...
void GlobalTxn::GetCellCallback(CellReaderContext* ctx) {
  ...
  bool last_cell = false;
  {
    ...
    if (internal_reader_ctx->fail_cell_cnt == 0 && ctx->status.GetType() == ErrorCode::kOK) {
      ...
      // return to user qualifier not qualifier.data
...
bool GlobalTxn::FindValueFromResultRow(RowReader::TRow& result_row, Cell* target_cell) {
  ...
  auto data_col_it = result_row[target_cell->ColFamily()].find(target_cell->DataName());
  ...
  auto data_col = result_row[target_cell->ColFamily()][target_cell->DataName()];

  for (auto k1 = write_col.rbegin(); k1 != write_col.rend(); ++k1) {
    int64_t write_ts = k1->first;
    std::string write_value = k1->second;
    VLOG(12) << "[gtxn][get][" << start_ts_ << "] found write col, ts=" << write_ts
             << ", internal val = " << write_value;
    int write_type;
    int64_t data_ts;
    ...
    VLOG(12) << "[gtxn][get][" << start_ts_ << "] decode write col, ts=" << write_ts
             << ", type=" << write_type << ", value=" << data_ts;
    // get data col , ts == data_ts
    for (auto k2 = data_col.rbegin(); k2 != data_col.rend(); ++k2) {
      VLOG(12) << "[gtxn][get][" << start_ts_ << "] found data col, ts=" << k2->first
               << ", internal val = " << k2->second;
      if (k2->first == data_ts && write_type == RowMutation::kPut) {
        ...
        VLOG(12) << "[gtxn][get][" << start_ts_ << "] data cell version not found, v=" << k2->first;
        break;
      }
    }
  ...
  VLOG(12) << "[gtxn][get][" << start_ts_ << "] write col versions count" << write_col.size();
  return false;
}

void GlobalTxn::BackoffAndMaybeCleanupLock(RowReader::TRow& row, const Cell& cell,
                                           const bool try_clean, ErrorCode* status) {
  VLOG(12) << gtxn_internal_->DebugString(
      cell, "[gtxn][get][" + std::to_string(start_ts_) + " backoff or cleanup lock");
  // get lock ts
  int64_t lock_ts = -1;
  int lock_type = -1;
  ...
  const std::string& process =
      "[gtxn][get][" + std::to_string(start_ts_) + "][check locked and writed]";
  bool ret = gtxn_internal_->PrimaryIsLocked(primary_info, lock_ts, status);
  lock_ts = lock_ts == -1 ? start_ts_ : lock_ts;
  if (status->GetType() != ErrorCode::kOK && status->GetType() != ErrorCode::kNotFound) {
    ...
    if (try_clean) {
      CleanLock(cell, primary_info, status, lock_ts);
    ...
    if (!gtxn_internal_->IsPrimary(cell, primary_info)) {
      ...
      if (status->GetType() == ErrorCode::kGTxnPrimaryLost) {
        VLOG(12) << gtxn_internal_->DebugString(cell,
                                                process + ", rollforward failed, try clean lock");
...
void GlobalTxn::CleanLock(const Cell& cell, const tera::PrimaryInfo& primary, ErrorCode* status,
  ...
  const Cell& primary_cell =
      Cell(primary_table, primary.row_key(), primary.column_family(), primary.qualifier());
  ...
  if (!is_same) {
    ...
    pri_mu->DeleteColumns(primary_cell.ColFamily(), primary_cell.LockName(), lock_ts);
  ...
  // delete all info between [0, start_ts_] at lock col
  this_mu->DeleteColumns(cell.ColFamily(), cell.LockName(), lock_ts);
  (cell.Table())->ApplyMutation(this_mu);
  if (this_mu->GetError().GetType() != tera::ErrorCode::kOK) {
    LOG(WARNING) << "[gtxn][get][" << start_ts_
                 << "] clean lock failed :" << this_mu->GetError().ToString();
    *status = this_mu->GetError();
  }
  delete this_mu;
}

void GlobalTxn::RollForward(const Cell& cell, const tera::PrimaryInfo& primary, int lock_type,
                            ErrorCode* status) {
  ...
  std::unique_ptr<Cell> primary_cell(
      new Cell(pri_table, primary.row_key(), primary.column_family(), primary.qualifier()));
  ...
  while (!reader->Done()) {
    ...
    VLOG(12) << "[gtxn][get][ " << start_ts_
             << "] decode primary 'write', ts=" << reader->Timestamp() << ", type=" << write_type
             << ", value=" << data_ts;
  ...
  if (commit_ts > 0) {
    RowMutation* this_mu = cell.Table()->NewRowMutation(cell.RowKey());
    this_mu->Put(cell.ColFamily(), cell.WriteName(), EncodeWriteValue(lock_type, data_ts),
                 commit_ts);
...
ErrorCode GlobalTxn::Commit() {
  ...
  if (put_fail_cnt_.Get() > 0 || has_commited_) {
    std::string reason("commit failed, has_commited[" + std::to_string(has_commited_.load()) +
...
void GlobalTxn::InternalCommit() {
  int64_t odd_time_ms = ttl_timestamp_ms_ - get_millis();
  if (odd_time_ms < commit_timeout_ms_) {
    commit_timeout_ms_ = odd_time_ms > 0 ? odd_time_ms : 1;
  }
  gtxn_internal_->SetCommitDuration(commit_timeout_ms_);
  ...
  primary_write_->Serialize(prewrite_start_ts_, gtxn_internal_->GetClientSession(),
                            &serialized_primary_);
...
void GlobalTxn::AsyncPrewrite(std::vector<Write>* ws) {
  ...
  PrewriteContext* ctx = new PrewriteContext(ws, this, single_row_txn, w.TableName(), w.RowKey());
...
//      b) write "lock" and "data" column to tera, through same single_row_txn
//      in step(1)
//
// call by [prewrite] step(1),through reader callback
//
void GlobalTxn::DoPrewriteReaderCallback(RowReader* r) {
  std::unique_ptr<RowReaderImpl> reader(static_cast<RowReaderImpl*>(r));
  PrewriteContext* ctx = (PrewriteContext*)reader->GetContext();
  Transaction* single_row_txn = ctx->stxn;
  ...
  if (reader->GetError().GetType() != ErrorCode::kNotFound &&
    ...
    gtxn_internal_->BuildRowMutationForPrewrite(ctx->ws, prewrite_mu, serialized_primary_);
    ...
    single_row_txn->SetCommitCallback([](Transaction* single_txn) {
      ...
      gtxn->thread_pool_->AddTask(std::bind(&GlobalTxn::DoPrewriteCallback, gtxn, single_txn));
    });
    if (gtxn_internal_->IsTimeOut()) {
      ctx->status.SetFailed(ErrorCode::kGTxnPrewriteTimeout, "global transaction prewrite timeout");
...
//
void GlobalTxn::DoPrewriteCallback(Transaction* single_row_txn) {
  ...
  if (gtxn_internal_->IsTimeOut() || status.GetType() != ErrorCode::kOK) {
    ...
    thread_pool_->AddTask(
        std::bind(&GlobalTxn::AsyncPrewrite, this, &(prewrite_iterator_->second)));
    delete ctx;
  } else {
...
void GlobalTxn::VerifyPrimaryLocked() {
  ...
  Transaction* pri_txn = pri_t->StartRowTransaction(primary_write_->RowKey());
  ...
  VLOG(12) << "[gtxn][vertify][get] " << primary_write_->DebugString()
           << " LockName:Type:prewrite_start_ts " << primary_write_->LockName() << ":"
           << primary_write_->WriteType() << ":" << prewrite_start_ts_;
  reader->SetTimeRange(prewrite_start_ts_, prewrite_start_ts_);
  PrimaryTxnContext* ctx = new PrimaryTxnContext(this, pri_txn);
  reader->SetContext(ctx);
  reader->SetCallBack([](RowReader* r) {
    GlobalTxn* gtxn = static_cast<GlobalTxn*>(((PrimaryTxnContext*)r->GetContext())->gtxn);
    gtxn->DoVerifyPrimaryLockedCallback(r);
  });
  pri_txn->Get(reader);
}
...
void GlobalTxn::DoVerifyPrimaryLockedCallback(RowReader* reader) {
  ErrorCode status = reader->GetError();
  PrimaryTxnContext* ctx = (PrimaryTxnContext*)reader->GetContext();
  Transaction* pri_txn = ctx->stxn;
  delete reader;

  if (status.GetType() == ErrorCode::kOK) {
    pri_txn->SetContext(ctx);
...
void GlobalTxn::CommitPrimary(Transaction* pri_txn) {
  Table* pri_t = primary_write_->Table();
  ...
  pri_txn->SetCommitCallback([](Transaction* txn) {
    GlobalTxn* gtxn = static_cast<GlobalTxn*>(((PrimaryTxnContext*)txn->GetContext())->gtxn);
    gtxn->CheckPrimaryStatusAndCommmitSecondaries(txn);
  });
  pri_txn->Commit();
  delete primary_mu;
}
...
void GlobalTxn::CheckPrimaryStatusAndCommmitSecondaries(Transaction* pri_txn) {
  std::unique_ptr<PrimaryTxnContext> ctx((PrimaryTxnContext*)pri_txn->GetContext());
  ...
  VLOG(12) << "[gtxn][commit] succeed :[" << start_ts_ << "," << prewrite_start_ts_ << ","
           << commit_ts_ << "]";
  ...
  for (auto& same_row_writes : writes_) {
    thread_pool_->AddTask(
        std::bind(&GlobalTxn::AsyncCommitSecondaries, this, &(same_row_writes.second)));
  }

  /// begin ack
  for (auto& same_row_acks : acks_) {
    thread_pool_->AddTask(std::bind(&GlobalTxn::AsyncAck, this, &(same_row_acks.second)));
  }
  /// begin notify
  for (auto& same_row_notifies : notifies_) {
    thread_pool_->AddTask(std::bind(&GlobalTxn::AsyncNotify, this, &(same_row_notifies.second)));
  }
  bool should_callback = false;
  {
    ...
    should_callback = commit_secondaries_done_cnt_.Get() == writes_cnt_.Get() &&
                      ...
                      notifies_cnt_.Get() == notify_done_cnt_.Get() && all_task_pushed_ == true;
  }
  if (should_callback) {
    RunUserCallback();
  }
...
void GlobalTxn::AsyncAck(std::vector<Write>* ws) {
  ...
  mu->SetCallBack(
      [](RowMutation* row_mu) { ((GlobalTxn*)row_mu->GetContext())->DoAckCallback(row_mu); });
  mu->SetContext(this);
...
void GlobalTxn::DoAckCallback(RowMutation* mutation) {
  if (mutation->GetError().GetType() != tera::ErrorCode::kOK) {
    LOG(WARNING) << "[gtxn][commit][ack], failed" << mutation->GetError().GetReason();
    ErrorCode status;
...
void GlobalTxn::AsyncNotify(std::vector<Write>* ws) {
  ...
  mu->SetCallBack(
      [](RowMutation* row_mu) { ((GlobalTxn*)row_mu->GetContext())->DoNotifyCallback(row_mu); });
  mu->SetContext(this);
  table->ApplyMutation(mu);
}
...
void GlobalTxn::DoNotifyCallback(RowMutation* mutation) {
  if (mutation->GetError().GetType() != tera::ErrorCode::kOK) {
    LOG(WARNING) << "[gtxn][commit][notify], failed" << mutation->GetError().GetReason();
    ErrorCode status;
...
void GlobalTxn::DoCommitSecondariesCallback(RowMutation* mutation) {
  if (mutation->GetError().GetType() != tera::ErrorCode::kOK) {
    LOG(WARNING) << "[gtxn][commit][secondaries], failed" << mutation->GetError().GetReason();
...
void GlobalTxn::ApplyMutation(RowMutation* row_mu) {
  ...
  if (!has_commited_.load()) {
    ...
    std::string reason =
        "ApplyMutation failed, txn has committed at [" + std::to_string(commit_ts_) + "]";
...
void GlobalTxn::Ack(Table* t, const std::string& row_key, const std::string& column_family,
...
void GlobalTxn::Notify(Table* t, const std::string& row_key, const std::string& column_family,
                       const std::string& qualifier) {
  if (t == NULL) {
    LOG(ERROR) << "set ack cell failed";
    return;
