...
RowTransactionImpl::RowTransactionImpl(TableImpl* table, const std::string& row_key,
    ...
    : _table(table),
      ...
      _read_snapshot_sequence(kMaxSequenceNumber),
...
void ReadCallbackWrapper(RowReader* row_reader) {
    ...
    if ((code == ErrorCode::kOK || code == ErrorCode::kNotFound)
            && reader_impl->GetTmpSnapshot() < kMaxSequenceNumber) {
        CHECK_EQ(txn_impl->_read_snapshot_sequence, kMaxSequenceNumber);
        CHECK_EQ(txn_impl->_last_read_sequence, kMaxSequenceNumber);
        CHECK_EQ(reader_impl->GetSnapshot(), 0u);
        txn_impl->_last_read_sequence = reader_impl->GetLastSequence();
        txn_impl->_read_snapshot_sequence = reader_impl->GetTmpSnapshot();
    }
...
void RowTransactionImpl::Get(RowReader* row_reader) {
    ...
    if (reader_impl->GetSnapshot() == 0) {
        if (_read_snapshot_sequence == kMaxSequenceNumber) {
            reader_impl->SetCreateTmpSnapshot();
        } else {
            reader_impl->SetTmpSnapshot(_read_snapshot_sequence);
        }
    }
