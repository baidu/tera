...
TEST_F(SdkTableTest, UnnormalLifeCycleReadTableMetaCallBack) {
  ...
  ThreadPool::Task task_ =
      std::bind(&TableImpl::ReadTableMetaCallBackWrapper,
                std::weak_ptr<TableImpl>(std::static_pointer_cast<TableImpl>(table_)), &ret_err, 0,
                &request, &response, false, 0);
...
TEST_F(SdkTableTest, UnnormalLifeCycleScanMetaTableCallBack) {
  ...
  ThreadPool::Task task_ =
      std::bind(&TableImpl::ScanMetaTableCallBackWrapper,
                std::weak_ptr<TableImpl>(std::static_pointer_cast<TableImpl>(table_)), empty_str,
                empty_str, empty_str, start_time, &request, &response, false, 0);
  table_->AddDelayTask(1 * 1000 /*ms*/, task_);
  table_.reset();
  sleep(2);
}
...
TEST_F(SdkTableTest, UnnormalLifeCycleReaderCallBack) {
  ...
  ReadTabletResponse response;
  ThreadPool::Task task_ =
      std::bind(&TableImpl::ReaderCallBackWrapper,
                std::weak_ptr<TableImpl>(std::static_pointer_cast<TableImpl>(table_)), &vec,
                &request, &response, false, 0);
  table_->AddDelayTask(1 * 1000 /*ms*/, task_);
  table_.reset();
  sleep(2);
...
TEST_F(SdkTableTest, UnnormalLifeCycleMutateCallBack) {
  ...
  ThreadPool::Task task_ =
      std::bind(&TableImpl::MutateCallBackWrapper,
                std::weak_ptr<TableImpl>(std::static_pointer_cast<TableImpl>(table_)), &vec,
                &request, &response, false, 0);
  table_->AddDelayTask(1 * 1000 /*ms*/, task_);
  table_.reset();
...
TEST_F(SdkTableTest, UnnormalLifeCycleScanCallBack) {
  ...
  ScanTabletResponse response;
  ThreadPool::Task task_ =
      std::bind(&TableImpl::ScanCallBackWrapper,
                std::weak_ptr<TableImpl>(std::static_pointer_cast<TableImpl>(table_)), &scan_task,
                &request, &response, false, 0);
  table_->AddDelayTask(1 * 1000 /*ms*/, task_);
  table_.reset();
  sleep(2);
