...
public:
    GlobalTxnTest() : 
        thread_pool_(2), 
        gtxn_(std::shared_ptr<ClientImpl>(), &thread_pool_, (new sdk::MockTimeoracleClusterFinder(""))) {
...
TEST_F(GlobalTxnTest, CheckPrimaryStatusAndCommmitSecondaries) {
    ...
    SingleRowTxn* txn = new SingleRowTxn(static_cast<TableWrapper*>(t1.get())->GetTableImpl(), "rowkey", NULL);
    PrimaryTxnContext* ctx = new PrimaryTxnContext(&gtxn_, txn);
    txn->SetContext(ctx);
...
TEST_F(GlobalTxnTest, Notify) {
    ...
    EXPECT_TRUE(gtxn_.notifies_cnt_.Get() == 0);
    for (int i = 0; i < notify_thread_cnt; ++i) {
        threads.emplace_back(std::thread(NotifyWarpper, &gtxn_, t1.get(), "r" + std::to_string(i), "", ""));
    }
    for (int i = 0; i < threads.size(); ++i) {
        threads[i].join();
    }
...
TEST_F(GlobalTxnTest, Ack) {
    ...
    EXPECT_TRUE(gtxn_.acks_cnt_.Get() == 0);
    for (int i = 0; i < ack_thread_cnt; ++i) {
        threads.emplace_back(std::thread(AckWarpper, &gtxn_, t1.get(), "r" + std::to_string(i), "", ""));
    }
    for (int i = 0; i < threads.size(); ++i) {
        threads[i].join();
    }
...
TEST_F(GlobalTxnTest, DoPrewriteCallback0) {
    ...
    SingleRowTxn* stxn = static_cast<TransactionWrapper<SingleRowTxn>*>(txn)->GetTransactionPtr().get();
    Cell cell(t.get(), "r1", "cf", "qu", 1, "val");
    Write w(cell);
    std::vector<Write> ws;
    ws.push_back(w);
