...
StatTable::StatTable(ThreadPool* thread_pool,
                     const StatTableCustomer& c,
...
void StatTable::SelectTabletsFailMessages(const std::string& ts_addr, 
    ...
    for (scanner->LookUp("!"); !scanner->Done(); scanner->Next()) {
        ...
        std::string corrupt_phase = 
            static_cast<CorruptPhase>(corrupt_msg.corrupt_phase()) == CorruptPhase::kLoading ? "Load" : "Comp";
        row.clear();
...
void StatTable::RecordTabletCorrupt(const std::string& tablet,
                                    const std::string& corrupt_msg) {
    if (!opened_) {
...
std::string StatTable::SerializeCorrupt(CorruptPhase phase,
...
void StatTable::RecordStatTableCallBack(RowMutation* mutation) {
    ...
    if (error_code.GetType() != ErrorCode::kOK) {
        LOG(WARNING) << "dump stat exception occured, reason:"
