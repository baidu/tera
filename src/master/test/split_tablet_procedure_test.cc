...
public:
    SplitTabletProcedureTest() : tablet_manager_(new TabletManager(nullptr, nullptr, nullptr)), 
    ...
    virtual void SetUp() {
        ...
        TabletManager::PackTabletMeta(&tablet_meta, "test", "b", "d", "test/tablet00000001", "", TabletMeta::kTabletOffline, 10);
...
//                    "b", "d", "test/tablet00000001", "", schema, 10, &tablet_, &ret_code));
...
TEST_F(SplitTabletProcedureTest, PostUnLoadTabletPhaseHandler) {
    ...
    std::string log_path = FLAGS_tera_tabletnode_path_prefix + 
            leveldb::LogHexFileName(tablet_->GetPath(), 123);
