...
class MergeTabletProcedureTest : public ::testing::Test {
 public:
  ...
  virtual void SetUp() {
    ...
    TabletManager::PackTabletMeta(&tablet_meta, "test", "a", "b", "test/tablet00000001", "",
                                  TabletMeta::kTabletOffline, 10);
    tablets_[0] = table_->AddTablet(tablet_meta, &ret_code);
    TabletManager::PackTabletMeta(&tablet_meta, "test", "b", "c", "test/tablet00000002", "",
                                  TabletMeta::kTabletOffline, 10);
    tablets_[1] = table_->AddTablet(tablet_meta, &ret_code);
 ...
 private:
  ...
  void InitFileSystemForMerge() {
    ...
    merge_proc_.reset(
        new MergeTabletProcedure(tablets_[0], tablets_[1], MasterEnv().GetThreadPool().get()));
  }
...
TEST_F(MergeTabletProcedureTest, MergeTabletProcedureInit) {
  ...
  merge_proc_.reset(
      new MergeTabletProcedure(tablets_[1], tablets_[0], MasterEnv().GetThreadPool().get()));
  EXPECT_EQ(merge_proc_->phases_.back(), MergeTabletPhase::kUnLoadTablets);
  TabletMeta tablet_meta;
  TabletManager::PackTabletMeta(&tablet_meta, "test", "ab", "c", "test/tablet00000001", "",
                                TabletMeta::kTabletOffline, 10);
  StatusCode ret_code;
  TabletPtr tablet = table_->AddTablet(tablet_meta, &ret_code);
  EXPECT_TRUE(tablet);
  tablet->SetStatus(TabletMeta::kTabletReady);
  merge_proc_.reset(
      new MergeTabletProcedure(tablets_[0], tablet, MasterEnv().GetThreadPool().get()));
