...
public:
    LoadTabletProcedureTest() : tablet_manager_(new TabletManager(nullptr, nullptr, nullptr)), 
    ...
    virtual void SetUp() {
        ...
        TabletManager::PackTabletMeta(&tablet_meta, 
                "test", "", "", "test/tablet00000001", "", TabletMeta::kTabletOffline, 0);
...
void LoadTabletProcedureTest::InitMasterEnv() {
    ...
    MasterEnv().Init(new MasterImpl, ts_manager_, tablet_manager_, 
            ...
            std::shared_ptr<tera::sdk::StatTable>(new tera::sdk::StatTable(nullptr, sdk::StatTableCustomer::kMaster)));
}
...
TEST_F(LoadTabletProcedureTest, GenerateEventForUserTabletInStatusOffLine) {
    ...
    tabletnode->SetState(kOffLine, NULL);
    ...
    tabletnode->SetState(kReady, NULL);
...
public:
    ...
    virtual bool UpdateRootTabletNode(const std::string& addr) {
        return true;
    }
};
...
TEST_F(LoadTabletProcedureTest, GenerateEventForMetaTablet) {
    TabletNodePtr tabletnode(new TabletNode("127.0.0.1:2000", "1234567"));
    MetaTabletPtr meta_tablet = tablet_manager_->AddMetaTablet(tabletnode, 
            std::shared_ptr<MasterZkAdapterBase>(new TestZkAdapter));
    ...
    EXPECT_EQ(tabletnode->GetState(), kPendingOffLine);
    EXPECT_EQ(load_proc_->GenerateEvent(), TabletEvent::kTsOffline);
    // meta tablet always load immediate discard tabletnode concurrency loading limits
    tabletnode->SetState(kReady, NULL);
...
TEST_F(LoadTabletProcedureTest, TestEventHandlers) {
    ...
    // tablet can only transite to kTableOnLoad from kTableOffLine, so set status to kTableOffLine first
    tablet_->SetStatus(TabletMeta::kTabletOffline);
...
// those AsycLifeCycle* test cases should cause core dump. Since load_proc_ is dectructed, we cannot 
