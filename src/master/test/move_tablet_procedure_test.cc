...
void MoveTabletProcedureTest::InitMasterEnv() {
  MasterEnv().Init(new MasterImpl(nullptr, nullptr), ts_manager_, tablet_manager_, access_builder_,
                   ...
                   tablet_availability_, nullptr);
}

TEST_F(MoveTabletProcedureTest, MoveProcedureInit) {
  tablet_->SetStatus(TabletMeta::kTabletReady);
  move_proc_ = std::shared_ptr<MoveTabletProcedure>(
      new MoveTabletProcedure(tablet_, nullptr, MasterEnv().GetThreadPool().get()));
  ...
  tablet_->SetStatus(TabletMeta::kTabletSplitted);
  move_proc_ = std::shared_ptr<MoveTabletProcedure>(
      new MoveTabletProcedure(tablet_, nullptr, MasterEnv().GetThreadPool().get()));
  EXPECT_EQ(move_proc_->phases_.back(), MoveTabletPhase::kEofPhase);
  tablet_->SetStatus(TabletMeta::kTabletMerged);
  move_proc_ = std::shared_ptr<MoveTabletProcedure>(
      new MoveTabletProcedure(tablet_, nullptr, MasterEnv().GetThreadPool().get()));
  EXPECT_EQ(move_proc_->phases_.back(), MoveTabletPhase::kEofPhase);
}
...
TEST_F(MoveTabletProcedureTest, UnLoadTabletPhaseHandler) {
  ...
  move_proc_->UnloadTabletPhaseHandler(MoveTabletPhase::kUnLoadTablet);
  // as the procedure_executor is not really running, tablet_ status will not
  // change
...
TEST_F(MoveTabletProcedureTest, EofPhaseHandler) {
  ...
  move_proc_ = std::shared_ptr<MoveTabletProcedure>(
      new MoveTabletProcedure(tablet_, dest_node_, MasterEnv().GetThreadPool().get()));
