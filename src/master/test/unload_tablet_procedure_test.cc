...
public:
    UnloadTabletProcedureTest() : tablet_manager_(new TabletManager(nullptr, nullptr, nullptr)), 
...
void UnloadTabletProcedureTest::InitMasterEnv() {
    MasterEnv().Init(nullptr, ts_manager_, tablet_manager_, 
            ...
            std::shared_ptr<ThreadPool>(new ThreadPool), nullptr, tablet_availability_, nullptr);
}
...
TEST_F(UnloadTabletProcedureTest, GenerateEvent) {
    ...
    FLAGS_tera_master_kick_tabletnode_enabled = true;
...
// those AsycLifeCycle* test cases should cause core dump. Since unload_proc_ is dectructed, we cannot 
// check its member fields
TEST_F(UnloadTabletProcedureTest, AsynLifeCycle_CallbackExceedProcedureCycle) {
    tablet_->SetStatus(TabletMeta::kTabletUnloading);
    tablet_->node_->SetState(kReady, NULL);
...
TEST_F(UnloadTabletProcedureTest, AsyncLifeCycleTest_DelayedUnloadTaskExeedProcedureCycle) {
    tablet_->SetStatus(TabletMeta::kTabletUnloading);
    tablet_->node_->SetState(kOffLine, NULL);
