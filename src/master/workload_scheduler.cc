...
class SizeComparator : public Comparator {
 public:
  int Compare(const TabletNodePtr& a, const TabletNodePtr& b, const std::string& table_name) {
...
bool SizeScheduler::MayMoveOut(const TabletNodePtr& node, const std::string& table_name) {
...
bool SizeScheduler::FindBestNode(const std::vector<TabletNodePtr>& node_list,
                                 const std::string& table_name, size_t* best_index) {
...
bool SizeScheduler::FindBestTablet(const TabletNodePtr& src_node, const TabletNodePtr& dst_node,
  ...
  for (size_t i = 0; i < tablet_list.size(); ++i) {
    ...
    if (size <= ideal_move_size && (best_tablet_index == -1 || size > best_tablet_size)) {
      best_tablet_index = i;
  ...
  *best_index = best_tablet_index;
  ...
  VLOG(16) << "[size-sched] best tablet = " << best_tablet->GetPath()
           << " size = " << best_tablet_size << " qps = " << best_tablet_qps;
  return true;
}
...
bool LoadScheduler::FindBestTablet(const TabletNodePtr& src_node, const TabletNodePtr& dst_node,
                                   const std::vector<TabletPtr>& tablet_list,
                                   const std::string& table_name, size_t* best_index) {
  ...
  *best_index = best_tablet_index;
  ...
  VLOG(16) << "[load-sched] best tablet = " << best_tablet->GetPath()
           << " size = " << best_tablet->GetDataSize() << " qps = " << best_tablet_qps;
  return true;
}
