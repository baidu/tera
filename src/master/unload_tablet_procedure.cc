...
std::map<TabletEvent, UnloadTabletProcedure::UnloadTabletEventHandler> 
    UnloadTabletProcedure::event_handlers_ {
        ...
        {TabletEvent::kWaitRpcResponse, std::bind(&UnloadTabletProcedure::WaitRpcResponseHandler, _1, _2)},
        {TabletEvent::kTsUnLoadSucc,    std::bind(&UnloadTabletProcedure::UnloadTabletSuccHandler, _1, _2)}, 
        {TabletEvent::kTsOffline,       std::bind(&UnloadTabletProcedure::UnloadTabletSuccHandler, _1, _2)}, 
        {TabletEvent::kTsUnLoadFail,    std::bind(&UnloadTabletProcedure::UnloadTabletFailHandler, _1, _2)},
        {TabletEvent::kEofEvent,        std::bind(&UnloadTabletProcedure::EOFHandler, _1, _2)}
    };

UnloadTabletProcedure::UnloadTabletProcedure(TabletPtr tablet, ThreadPool* thread_pool, bool is_sub_proc) : 
    ...
    PROC_LOG(INFO) << "unload tablet begin, tablet: " << tablet_->GetPath();
    // I played a trick here by setting tablet status to kTableReady when user want to unload a 
    ...
    PROC_CHECK(tablet_->GetStatus() == TabletMeta::kTabletReady);
}

std::string UnloadTabletProcedure::ProcId() const {
    return id_;
}
...
TabletEvent UnloadTabletProcedure::GenerateEvent() {
    ...
    if (tablet_->GetStatus() == TabletMeta::kTabletUnloading) {
        // if kick_tablenode_enabled, wait until the TS is kicked and TS->NodeDown() is true
        ...
        else if (FLAGS_tera_master_kick_tabletnode_enabled) {
            return TabletEvent::kWaitRpcResponse;
        }
...
void UnloadTabletProcedure::TsUnloadBusyHandler(const TabletEvent& event) {
    // only log once the first time we enter this handler, avoiding redundant logging
...
void UnloadTabletProcedure::UnloadTabletHandler(const TabletEvent& event) {
    ...
    PROC_LOG(INFO) << "dispatch UNLOAD tablet request to ts, tablet: " 
                << tablet_ << ", server: " << tablet_->GetServerAddr();
    UnloadTabletAsync();
}
...
// currently we will never enter this handler as we always kick tabletnode once all unload tries failed
...
void UnloadTabletProcedure::UnloadTabletAsyncWrapper(std::weak_ptr<UnloadTabletProcedure> weak_proc) {
...
void UnloadTabletProcedure::UnloadTabletCallback(UnloadTabletRequest* request, 
    ...
    if (node->NodeDown() || 
            (!failed && (status == kTabletNodeOk || status == kKeyNotInRange))){
    ...
    PROC_LOG(WARNING) << "unload tablet failed, " << tablet_
            << ", " << unload_retrys_ << "th attemp, error: " << errmsg;
   ...
   // failed and has no more retry times, return immediately
    ...
    if (unload_retrys_ > FLAGS_tera_master_impl_retry_times) {
        ...
        if (FLAGS_tera_master_kick_tabletnode_enabled) {
            PROC_LOG(ERROR) << kSms << "unload tablet failed finally, " << tablet_ 
                    << "now try kick ts: " << tablet_->GetTabletNode()->GetAddr();
            MasterEnv().GetMaster()->TryKickTabletNode(tablet_->GetTabletNode()->GetAddr());
        }
        else {
            PROC_LOG(ERROR) << kSms << "Unload tablet failed finally, " << tablet_;
        }
    ...
    ThreadPool::Task task = 
        std::bind(&UnloadTabletProcedure::UnloadTabletAsyncWrapper, 
