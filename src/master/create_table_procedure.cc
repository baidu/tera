...
std::map<CreateTablePhase, CreateTableProcedure::CreateTablePhaseHandler> CreateTableProcedure::phase_handlers_ {
...
void CreateTableProcedure::RunNextStage() {
    ...
    PROC_CHECK(it != phase_handlers_.end()) << "illegal phase: " << phase << ", table: " << table_name_;
...
void CreateTableProcedure::PreCheckHandler(const CreateTablePhase&) {
    {
        ...
        if (FLAGS_tera_acl_enabled && 
            !MasterEnv().GetMaster()->IsRootUser(request_->user_token()) && 
    ...
    MasterEnv().GetTabletManager()->AddTable(table_, &status);
    ...
    for (int32_t i = 1; i <= tablet_num; ++i) {
        ...
        TabletManager::PackTabletMeta(&meta, table_name, start_key, end_key, path, "", 
                TabletMeta::kTabletOffline, FLAGS_tera_tablet_write_block_size * 1024);
        TabletPtr tablet = TabletManager::CreateTablet(meta);
        tablet->LockTransition();
        if (!table_->AddTablet(tablet, &status)) {
            PROC_LOG(WARNING) << "add tablet failed" << tablet->GetPath();
    ...
    PROC_LOG(INFO) << "Begin to create table: " << request_->table_name()
        ...
        << ", raw_key: " << request_->schema().raw_key()
        << ", has " << tablet_num << " tablets, schema: "
        << request_->schema().ShortDebugString();
...
void CreateTableProcedure::LoadTabletsHandler(const CreateTablePhase&) {
    ...
    for (size_t i = 0; i < tablets_.size(); i++) {
        ...
        if (!MasterEnv().GetTabletNodeManager()
                ->ScheduleTabletNodeOrWait(size_scheduler, table_name_, false, &dest_node)) {
...
void CreateTableProcedure::EofHandler(const CreateTablePhase&) {
    ...
    done_.store(true);
    // unlike DisableTableProcedure, here we do not ensure that all tablets been loaded successfully 
    // and just finish the CreateTableProcedure early as all LoadTabletProcedure been added to ProcedureExecutor. 
...
bool CreateTableProcedure::Done() {
    return done_;
}

std::ostream& operator<< (std::ostream& o, const CreateTablePhase& phase) {
    static const char* msg[] = {"CreateTablePhase::kPrepare", 
                                "CreateTablePhase::kUpdateMeta",
                                "CreateTablePhase::kLoadTablets",
                                "CreateTablePhase::kEofPhase", 
    ...
    uint32_t index = static_cast<UnderType>(phase) - static_cast<UnderType>(CreateTablePhase::kPrepare);
