...
std::map<MergeTabletPhase, MergeTabletProcedure::MergeTabletPhaseHandler> MergeTabletProcedure::phase_handlers_ {
    {MergeTabletPhase::kUnLoadTablets,     std::bind(&MergeTabletProcedure::UnloadTabletsPhaseHandler, _1, _2)},
    {MergeTabletPhase::kPostUnLoadTablets, std::bind(&MergeTabletProcedure::PostUnloadTabletsPhaseHandler, _1, _2)},
    {MergeTabletPhase::kUpdateMeta,        std::bind(&MergeTabletProcedure::UpdateMetaPhaseHandler, _1, _2)},
    {MergeTabletPhase::kLoadMergedTablet,  std::bind(&MergeTabletProcedure::LoadMergedTabletPhaseHandler, _1, _2)},
    {MergeTabletPhase::kFaultRecover,      std::bind(&MergeTabletProcedure::FaultRecoverPhaseHandler, _1, _2)},
    {MergeTabletPhase::kEofPhase,          std::bind(&MergeTabletProcedure::EOFPhaseHandler, _1, _2)}
};

MergeTabletProcedure::MergeTabletProcedure(TabletPtr first, TabletPtr second, ThreadPool* thread_pool) : 
    id_(std::string("MergeTablet:") + first->GetPath() + ":" + TimeStamp()),
    tablets_{first, second}, thread_pool_(thread_pool)
{
    PROC_LOG_IF(INFO, tablets_[1]) << "merge tablet begin, tablets, first: " 
        << tablets_[0] << ", second: " << tablets_[1];
    ...
    if (tablets_[0]->GetKeyEnd() != tablets_[1]->GetKeyStart() && 
        ...
        PROC_LOG(WARNING) << "invalid merge peers: first: " << tablets_[0] << ", second: " << tablets_[1];
        SetNextPhase(MergeTabletPhase::kEofPhase);
        return;
    }
...
std::string MergeTabletProcedure::ProcId() const {
    //std::string id("MergeTablet:");
    //id = id + tablets_[0]->GetPath();
    return id_;
}
...
void MergeTabletProcedure::RunNextStage() {
    ...
    PROC_CHECK(it != phase_handlers_.end()) << "illegal phase: " << phase << ", tablet: " << tablets_[0];
...
void MergeTabletProcedure::UnloadTabletsPhaseHandler(const MergeTabletPhase&) {
    if (!unload_procs_[0] && !unload_procs_[1]) {
        ...
        PROC_LOG(INFO) << "Generate UnloadTablet SubProcedure1: " 
            << unload_procs_[0]->ProcId() << " SubProcedure2: " << unload_procs_[1]->ProcId();
    ...
    if (status0 != TabletMeta::kTabletOffline || status1 != TabletMeta::kTabletOffline) {
        PROC_LOG(WARNING) << "unload tablets not ok, tablet: " << tablets_[0]
            << ", status: " << StatusCodeToString(status0) << ", tablet: " 
            << tablets_[1] << ", status: " << StatusCodeToString(status1);
        SetNextPhase(MergeTabletPhase::kEofPhase);
    }
    else {
...
void MergeTabletProcedure::LoadMergedTabletPhaseHandler(const MergeTabletPhase&) {
    if (!load_proc_) {
        load_proc_.reset(new LoadTabletProcedure(merged_, dest_node_, thread_pool_));
...
void MergeTabletProcedure::FaultRecoverPhaseHandler(const MergeTabletPhase&) {
    ...
    if (!recover_procs_[0]) {
        recover_procs_[0].reset(new LoadTabletProcedure(tablets_[0],
                                tablets_[0]->GetTabletNode(), thread_pool_));
        recover_procs_[1].reset(new LoadTabletProcedure(tablets_[1],
                                tablets_[1]->GetTabletNode(), thread_pool_));
        MasterEnv().GetExecutor()->AddProcedure(recover_procs_[0]);
        MasterEnv().GetExecutor()->AddProcedure(recover_procs_[1]);
        PROC_LOG(INFO) << "[merge] rollback " << tablets_[0] << ", SubProcedure: " << recover_procs_[0]->ProcId();
        PROC_LOG(INFO) << "[merge] rollback " << tablets_[1] << ", SubProcedure: " << recover_procs_[1]->ProcId();
        return;
    }
    SetNextPhase(MergeTabletPhase::kEofPhase);
}
...
void MergeTabletProcedure::EOFPhaseHandler(const MergeTabletPhase&) {
    ...
    PROC_LOG(INFO) << "MergeTabletProcedure done";
    done_ = true;
}
...
bool MergeTabletProcedure::TabletStateCheck() {
    ...
    for (size_t i = 0; i < sizeof(tablets_)  / sizeof (TabletPtr); ++i) {
        ...
        for (size_t j = 0; j < children.size(); ++j) {
            ...
            uint64_t number = 0;
            if (ParseFileName(children[j], &number, &type) &&
...
void MergeTabletProcedure::UpdateMeta() {
    ...
    dest_node_ = (tablets_[0]->GetDataSize() > tablets_[1]->GetDataSize() ? 
        tablets_[0]->GetTabletNode() : tablets_[1]->GetTabletNode());
    ...
    PROC_LOG(INFO) << "[merge] update meta, tablet: [" << tablets_[0]->GetPath() << ", " << tablets_[1]->GetPath() << "]";
    // update meta table asynchronously until meta write ok.
    MasterEnv().BatchWriteMetaTableAsync(records, done, -1);
}

// will be called when update meta finish successfully, and set next process phase be LOAD_MERGED_TABLET
...
std::ostream& operator<< (std::ostream& o, const MergeTabletPhase& phase) {
    static const char* msg[] = {"MergeTabletPhase::kUnLoadTablets", 
    ...
    uint32_t index = static_cast<UnderType>(phase) - static_cast<UnderType>(MergeTabletPhase::kUnLoadTablets);
