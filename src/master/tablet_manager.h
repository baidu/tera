...
public:
    MetaTablet(const TabletMeta& meta, TablePtr table, std::shared_ptr<MasterZkAdapterBase> zk_adapter);
    // MetaTablet overwrite virtual function DoStateTransition() as it should update
    ...
    public:
        TableMetric(const std::string& name):
            ...
            table_size_("tera_master_table_size", GetTableNameLabel(),
                        {SubscriberType::LATEST}, false),
            corrupt_num_("tera_master_table_corrupt", GetTableNameLabel(),
                         {SubscriberType::LATEST}, false)
            {}
        ...
        void SetCorruptNum(int64_t corrupt_num) {
            corrupt_num_.Set(corrupt_num);
        }
...
public:
    ...
    bool AddTablet(TabletPtr& tablet, StatusCode* ret_code);
...
public:
    ...
    static TablePtr CreateTable(const std::string& table_name, const TableSchema& schema, const TableStatus& status);

    static TabletPtr CreateTablet(const TabletMeta& meta);

    static TabletPtr CreateTablet(TablePtr table, const TabletMeta& meta);
    static void PackTabletMeta(TabletMeta* meta, const std::string& table_name,
