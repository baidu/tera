...
void TeraMasterEnv::BatchWriteMetaTableAsync(MetaWriteRecord record, UpdateMetaClosure done, int32_t left_try_times) {
...
void TeraMasterEnv::BatchWriteMetaTableAsync(std::vector<MetaWriteRecord> meta_entries,
    ...
    WriteClosure meta_done = 
        std::bind(TeraMasterEnv::UpdateMetaCallback, meta_entries, done, left_try_times, _1, _2, _3, _4);
...
void TeraMasterEnv::UpdateMetaCallback(std::vector<MetaWriteRecord> records, 
    ...
    if (failed || status != kTabletNodeOk) {
        ...
        for (auto it = records.begin(); it != records.end(); ++it) {
            ...
            LOG(WARNING) << "update meta records suspended and retry later, " << "OP: " 
                << op << ", key: " << DebugString(it->key) << ", value: " << DebugString(it->value);
        }
        if (left_try_times == 0) {
            done(false);
        }
    ...
    for (auto it = records.begin(); it != records.end(); ++it) {
        std::string op = (it->is_delete ?  "DEL" : "PUT");
        LOG(INFO) << "update meta tablet succ, " << "OP: " << op 
            << ", key: " << DebugString(it->key) << ", value: " << DebugString(it->value);
    }
    done(true);
}
...
void TeraMasterEnv::SuspendMetaOperation(MetaWriteRecord record, UpdateMetaClosure done, int32_t left_try_times) {
    std::vector<MetaWriteRecord> meta_entries;
    meta_entries.push_back(record);
    SuspendMetaOperation(meta_entries, done, left_try_times);
}

void TeraMasterEnv::SuspendMetaOperation(std::vector<MetaWriteRecord> meta_entries, UpdateMetaClosure done, int32_t left_try_times) {
...
void TeraMasterEnv::PushToMetaPendingQueue(MetaTask* task) {
    ...
    if (meta_task_queue_.size() == 1) {
        ...
        MasterEnv().GetTabletManager()->FindTablet(FLAGS_tera_master_meta_table_name, "", 
                &meta_tablet);
...
void TeraMasterEnv::ResumeMetaOperation() {
    ...
    while (!meta_task_queue_.empty()) {
        ...
        if (task->type_ == kWrite) {
            ...
            BatchWriteMetaTableAsync(write_task->meta_entries_, write_task->done_, write_task->left_try_times_);
            delete write_task;
