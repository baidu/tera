...
public:
    ...
    // add a new transition rule representing object's state transfer from "curr_state" to "dest_state" driven by "event"
    StateTransitionRules& AddTransitionRule(const StateType& curr_state, 
    ...
    // return true and the associated post_state if there is a valid transition rule for <curr_state, event> 
    // else return false
    bool DoStateTransition(const StateType& curr_state, const EventType& event, StateType* post_state) const;

private:
    // this map is used to save all transition rules
    // key represent PrevState, value of type std::map<EventType, SateType> represent all events supported by PreState
...
template<typename StateType, typename EventType> 
bool StateTransitionRules<StateType, EventType>::
    DoStateTransition(const StateType& curr_state, const EventType& event, StateType* post_state) const {
