...
enum class TabletEvent {
    ...
    // a special event somewhat equivalent to TS_OFFLINE with consideration for cache locality
    kTsDelayOffline,      
    kTableDisable,          // enable table
    kTableEnable,           // disable table
    kEofEvent,              // end-marker event, some cleanup work maybe triggered by this event
};
...
public:
    ...
    // return true and update curr_state_ to the associated PostState corresponding to  
    // <curr_state_, event> pair if there is a valid transition rule for <curr_state_, event> pair, 
    // else return false;
    ...
    // get tablet's readytime in us, return int64::max() if tablet not in kTableReady status
    int64_t ReadyTime();

    typedef StateTransitionRules<TabletMeta::TabletStatus, TabletEvent> TabletStateTransitionRulesType;
private:
