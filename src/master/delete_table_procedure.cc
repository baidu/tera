...
std::map<DeleteTablePhase, DeleteTableProcedure::DeleteTablePhaseHandler> 
    DeleteTableProcedure::phase_handlers_ {
        {DeleteTablePhase::kPrepare, 
                std::bind(&DeleteTableProcedure::PrepareHandler, _1, _2)},
        {DeleteTablePhase::kDeleteTable, 
                std::bind(&DeleteTableProcedure::DeleteTableHandler, _1, _2)},
        {DeleteTablePhase::kUpdateMeta, 
                std::bind(&DeleteTableProcedure::UpdateMetaHandler, _1, _2)},
        {DeleteTablePhase::kEofPhase, 
                std::bind(&DeleteTableProcedure::EofPhaseHandler, _1, _2)}
};

DeleteTableProcedure::DeleteTableProcedure(TablePtr table, 
...
void DeleteTableProcedure::RunNextStage() {
    ...
    PROC_CHECK(it != phase_handlers_.end()) 
        << "illegal phase:" << phase << ", table: " << table_->GetTableName();
...
void DeleteTableProcedure::DeleteTableHandler(const DeleteTablePhase&) {
    ...
    for (size_t i = 0; i < tablets.size(); ++i) {
        ...
        if (tablet->GetStatus() != TabletMeta::kTabletDisable) {
            PROC_LOG(WARNING) << "tablet: " << tablet 
                << " not in disabled status, " << StatusCodeToString(tablet->GetStatus());
...
std::ostream& operator<< (std::ostream& o, const DeleteTablePhase& phase) {
    static const char* msg[] = {"DeleteTablePhase::kPrepare", 
                                "DeleteTablePhase::kDeleteTable", 
                                "DeleteTablePhase::kUpdateMeta", 
                                "DeleteTablePhase::kEofPhase", 
    ...
    uint32_t index = static_cast<UnderType>(phase) - static_cast<UnderType>(DeleteTablePhase::kPrepare);
