...
public:
    ...
    virtual void OnNotify(tera::Transaction* t,
        ...
        table_name << ":" << family << ":" <<
        qualifier << " row=" << row <<
        " value=" << value << " timestamps[0]=" << value;
...
public:
    TestClient() : ClientImpl(ClientOptions(), NULL, NULL),
                   thread_pool_(5) {}
    ~TestClient() {}
...
TEST(ScannerImpl, NextRow) {
    ...
    // stream done
    EXPECT_FALSE(scanner.NextRow(result_stream.get(), "table_name", &finished, &vec_rowkey, &vec_col));
...
TEST(ScannerImpl, CheckTransactionTypeLegalForTable) {
    ScannerImpl scanner;
    EXPECT_EQ(scanner.CheckTransactionTypeLegalForTable(kGlobalTransaction, kGlobalTransaction), true);
    EXPECT_EQ(scanner.CheckTransactionTypeLegalForTable(kSingleRowTransaction, kSingleRowTransaction), true);
    ...
    EXPECT_EQ(scanner.CheckTransactionTypeLegalForTable(kNoneTransaction, kSingleRowTransaction), true);

    EXPECT_EQ(scanner.CheckTransactionTypeLegalForTable(kSingleRowTransaction, kNoneTransaction), false);
    EXPECT_EQ(scanner.CheckTransactionTypeLegalForTable(kSingleRowTransaction, kGlobalTransaction), false);

    EXPECT_EQ(scanner.CheckTransactionTypeLegalForTable(kGlobalTransaction, kNoneTransaction), false);
    EXPECT_EQ(scanner.CheckTransactionTypeLegalForTable(kGlobalTransaction, kSingleRowTransaction), false);
}
