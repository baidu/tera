...
public:
    TestTxn(Table* table, const std::string& row_key,
            common::ThreadPool* thread_pool, int64_t start_ts = 0)
...
TEST_F(ObserverImplTest, ValidateCellValue) {
    ValidateCellValueTest();
}

TEST_F(ObserverImplTest, ValidateAckConfilict) {
    ValidateAckConfilictTest();
}
