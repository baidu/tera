...
TeraTpccDb::~TeraTpccDb() { delete client_; }
...
bool TeraTpccDb::InsertItem(const Item& i) {
  ...
  if (gtxn->GetError().GetType() != ErrorCode::kOK) {
    LOG(ERROR) << "insert table:" << tablename << " failed. err:" << gtxn->GetError().ToString();
    delete gtxn;
    return false;
  }
  delete gtxn;
...
bool TeraTpccDb::InsertCustomer(const Customer& c) {
  ...
  Transaction* gtxn = client_->NewGlobalTransaction();
  std::string key = std::to_string(c.c_w_id) + "_" + std::to_string(c.c_d_id) + "_" + c.c_last +
                    "_" + std::to_string(c.c_id);
...
bool TeraTpccDb::InsertHistory(const History& h) {
  ...
  if (hindex_reader->GetError().GetType() != ErrorCode::kNotFound &&
      !GetValues(&ret, gtxn, hindex_reader, {"count"}, &hindex_ret,
...
bool TeraTpccDb::GetValues(TxnResult* ret, Transaction* gtxn, RowReader* reader,
                           ...
                           RetTuples* ret_tuples, const std::string& if_error_msg) {
...
bool TeraTpccDb::GetCustomer(TxnResult* ret, Transaction* gtxn, bool by_last_name,
                             ...
                             int32_t warehouse_id, int32_t district_id, std::string* customer_key,
  ...
  *customer_key = std::to_string(warehouse_id) + "_" + std::to_string(district_id) + "_";
  ...
  if (by_last_name) {
    ...
    for (scanner->LookUp(start_key); !scanner->Done(); scanner->Next()) {
      ...
      RetTuples index_ret;
      if (!GetValues(ret, gtxn, index_reader, {"c_id"}, &index_ret,
  ...
  if (!GetValues(
          ret, gtxn, customer_reader,
          {"c_id", "c_d_id", "c_w_id", "c_first", "c_middle", "c_last", "c_balance",
           "c_ytd_payment", "c_payment_cnt", "c_credit", "c_data", "c_street_1", "c_street_2",
           "c_city", "c_state", "c_zip", "c_phone", "c_since", "c_credit_lim", "c_discount"},
          customer_ret, "@get_customer|customer_reader" + *customer_key)) {
    return false;
  }
  return true;
}
