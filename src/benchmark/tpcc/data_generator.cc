...
void DataGenerator::GenCustomers(int32_t district_id, int32_t warehouse_id) {
  IdSet bad_credit_ids =
      PickUniqueIdSet(rand_gen_, kCustomerCountPerDistrict / 10, 1, kCustomerCountPerDistrict);
  ...
  for (int c_id = 1; c_id <= kCustomerCountPerDistrict; ++c_id) {
    ...
    VLOG(12) << c.ToString();
    db_->InsertCustomer(c) ? states_[kCustomerTable].first.Inc()
                           : states_[kCustomerTable].second.Inc();
  }
...
void DataGenerator::GenHistorys(int32_t district_id, int32_t warehouse_id) {
  ...
  for (int h_id = 1; h_id <= kCustomerCountPerDistrict; ++h_id) {
    ...
    VLOG(12) << h.ToString();
    db_->InsertHistory(h) ? states_[kHistoryTable].first.Inc()
                          : states_[kHistoryTable].second.Inc();
  }
  event_.Complete(kCustomerCountPerDistrict);
}
...
void DataGenerator::GenOrderLines(int cnt, int32_t order_id, int32_t district_id,
  ...
  for (int i = 1; i <= cnt; ++i) {
    ...
    VLOG(12) << ol.ToString();
    db_->InsertOrderLine(ol) ? states_[kOrderLineTable].first.Inc()
                             : states_[kOrderLineTable].second.Inc();
  }
  event_.Complete(cnt);
}
...
void DataGenerator::GenOrders(int32_t d_id, int32_t w_id) {
  ...
  for (int o_id = 1; o_id <= kCustomerCountPerDistrict; ++o_id) {
    ...
    if (new_order) {
      ...
      VLOG(12) << no.ToString();
      db_->InsertNewOrder(no) ? states_[kNewOrderTable].first.Inc()
                              : states_[kNewOrderTable].second.Inc();
      event_.Complete(1);
    }
...
void DataGenerator::GenDistricts(int32_t warehouse_id) {
  ...
  for (int d_id = 1; d_id <= kDistrictCountPerWarehouse; ++d_id) {
    ...
    VLOG(12) << d.ToString();
    db_->InsertDistrict(d) ? states_[kDistrictTable].first.Inc()
                           : states_[kDistrictTable].second.Inc();
...
void DataGenerator::GenWarehouses() {
  ...
  for (int32_t w_id = 1; w_id <= FLAGS_warehouses_count; ++w_id) {
    ...
    VLOG(12) << w.ToString();
    db_->InsertWarehouse(w) ? states_[kWarehouseTable].first.Inc()
                            : states_[kWarehouseTable].second.Inc();
