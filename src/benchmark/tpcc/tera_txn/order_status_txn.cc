...
void TeraTpccDb::OrderStatusTxn(bool by_last_name, int32_t warehouse_id, int32_t district_id,
  ...
  if (!GetCustomer(ret, gtxn.get(), by_last_name, last_name, c_customer_id, warehouse_id,
                   district_id, &customer_key, &customer_ret)) {
    return;
  }

  // find newest order from order index
  ErrorCode error_code;
  std::string prefix_key = std::to_string(warehouse_id) + "_" + std::to_string(district_id) + "_";
  ...
  for (scanner->LookUp(start_key); !scanner->Done(); scanner->Next()) {
    ...
    RetTuples index_ret;
    if (!GetValues(ret, gtxn.get(), index_reader, {"o_id"}, &index_ret,
  ...
  RetTuples order_ret;
  if (!GetValues(ret, gtxn.get(), order_reader, {"o_ol_cnt", "o_id"}, &order_ret,
  ...
  for (int32_t i = 1; i <= std::stoi(order_ret["o_ol_cnt"]); ++i) {
    ...
    RetTuples ol_ret;
    if (!GetValues(ret, gtxn.get(), ol_reader, {},  // TODO
                   &ol_ret, "@order_status|ol_reader|" + ol_key)) {
      return;
    }
  }
