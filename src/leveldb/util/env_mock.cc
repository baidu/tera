...
static std::string MockPath(const std::string& o) { return mock_path_prefix + o; }

void MockEnv::SetPrefix(const std::string& p) { mock_path_prefix = p + "/"; }
...
void MockEnv::SetSequentialFileReadCallback(bool (*p)(int32_t i, char* scratch,
                                                      size_t* mock_size)) {
...
class MockSequentialFile : public SequentialFile {
 ...
 public:
  MockSequentialFile(const std::string& fname, FILE* f) : filename_(fname), file_(f) {}
...
class MockWritableFile : public WritableFile {
 ...
 public:
  MockWritableFile(const std::string& fname, FILE* f) : filename_(fname), file_(f) {}
...
class MockRandomAccessFile : public RandomAccessFile {
 ...
 public:
  MockRandomAccessFile(const std::string& fname, int fd) : filename_(fname), fd_(fd) {}
...
// writable
Status MockEnv::NewWritableFile(const std::string& fname, WritableFile** result,
                                const EnvOptions&) {
  Status s;
...
Status MockEnv::UnlockFile(FileLock* lock) { return Status::OK(); }
