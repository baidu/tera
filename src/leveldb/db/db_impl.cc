  ...
  *exists = true;
  ...
  if (options_.parent_tablets.size() == 0) {
    ...
    *exists = false;
    ...
    s = env_->FileExists(current);
    if (!s.ok()) {
      // maybe lost, maybe timeout, maybe permission denied, maybe ...
      Log(options_.info_log, "[%s] parent tablet(%ld) current error: %s",
          dbname_.c_str(),
          static_cast<long>(options_.parent_tablets[0]), s.ToString().c_str());
      return Status::IOError("parent CURRENT error");
    }
    *exists = true;
    return s;
    ...
    std::string current0 =
        CurrentFileName(RealDbName(dbname_, options_.parent_tablets[0]));
    s = env_->FileExists(current0);
    if (!s.ok()) {
      Log(options_.info_log, "[%s] parent tablet-0(%ld) current error: %s",
          dbname_.c_str(),
          static_cast<long>(options_.parent_tablets[0]), s.ToString().c_str());
      return Status::IOError("parent CURRENT error");
    }
    ...
    s = env_->FileExists(current1);
    if (!s.ok()) {
      Log(options_.info_log, "[%s] parent tablet-1(%ld) current error: %s",
          dbname_.c_str(),
          static_cast<long>(options_.parent_tablets[1]), s.ToString().c_str());
      return Status::IOError("parent CURRENT error");
    }
    *exists = true;
    return Status::OK();
