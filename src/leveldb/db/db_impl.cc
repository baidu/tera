...
Status DBImpl::Get(const ReadOptions& options,
  ...
  // Unlock while reading from files and memtables
  {
    ...
    if (mem->Get(lkey, value, sequence_number, options.rollbacks, &s)) {
      // Done
    } else if (imm != NULL && imm->Get(lkey, value, sequence_number, options.rollbacks, &s)) {
      // Done
    } else {
      s = current->Get(options, lkey, value, sequence_number, &stats);
