...
Status DirectBuildTable(const std::string& dbname, int32_t file_no, Env* env,
...
// directly build db
Status DirectBuildDB(const std::string& dbname, const Options& options) {
  DB* db = NULL;
  Status s;

  s = DB::Open(options, dbname, &db);
  if (!s.ok()) {
    std::cerr << "fail to open db: " << dbname << ", error: " << s.ToString() << std::endl;
    return s;
  }
