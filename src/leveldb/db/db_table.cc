...
struct DBTable::RecordWriter {
  ...
  explicit RecordWriter(port::Mutex* mu) : batch(NULL), sync(true), done(false), cv(mu) {}
};
...
Options InitDefaultOptions(const Options& options, const std::string& dbname) {
  ...
  if (!s.ok()) {
    std::cerr << "[" << dbname << "] fail to create dir: " << s.ToString() << std::endl;
  }
  if (opt.info_log == NULL) {
    s = opt.env->NewLogger(InfoLogFileName(dbname), LogOption::LogOptionBuilder().Build(),
                           &opt.info_log);
    if (!s.ok()) {
      // No place suitable for logging
      std::cerr << "[" << dbname << "] fail to init info log: " << s.ToString() << std::endl;
      opt.info_log = NULL;
    }
  }
  assert(s.ok());
...
Options InitOptionsLG(const Options& options, uint32_t lg_id) {
  ...
  opt.persistent_cache = lg_info->persistent_cache;
  ...
  opt.memtable_shard_num = lg_info->memtable_shard_num;
...
DBTable::DBTable(const Options& options, const std::string& dbname)
    : state_(kNotOpen),
      shutting_down_(NULL),
      shutdown1_finished_(NULL),
      bg_cv_(&mutex_),
      bg_cv_timer_(&mutex_),
      bg_cv_sleeper_(&mutex_),
      ...
      created_own_compact_strategy_(options_.compact_strategy_factory !=
                                    options.compact_strategy_factory),
      ...
      bg_schedule_gc_id_(0),
      bg_schedule_gc_score_(0),
      force_clean_log_seq_(0) {}

Status DBTable::Shutdown1() {
  assert(state_ == kOpened);
  state_ = kShutdown1;

  LEVELDB_LOG(options_.info_log, "[%s] shutdown1 start", dbname_.c_str());
  ...
  LEVELDB_LOG(options_.info_log, "[%s] wait bg garbage clean finish", dbname_.c_str());
  mutex_.Lock();
  if (bg_schedule_gc_) {
  ...
  LEVELDB_LOG(options_.info_log, "[%s] fg garbage clean", dbname_.c_str());
  GarbageClean();

  LEVELDB_LOG(options_.info_log, "[%s] shutdown1 done", dbname_.c_str());
...
Status DBTable::Shutdown2() {
  ...
  LEVELDB_LOG(options_.info_log, "[%s] shutdown2 start", dbname_.c_str());
  ...
  LEVELDB_LOG(options_.info_log, "[%s] stop async log", dbname_.c_str());
  if (log_) {
    log_->Stop(false);
  }

  if (s.ok() && options_.dump_mem_on_shutdown) {
    LEVELDB_LOG(options_.info_log, "[%s] gather all log file", dbname_.c_str());
    std::vector<uint64_t> logfiles;
    s = GatherLogFile(0, &logfiles);
    if (s.ok()) {
      LEVELDB_LOG(options_.info_log, "[%s] delete all log file", dbname_.c_str());
      s = DeleteLogFile(logfiles);
    }
  }

  LEVELDB_LOG(options_.info_log, "[%s] shutdown2 done", dbname_.c_str());
  return s;
}
...
Status DBTable::Init() {
  ...
  LEVELDB_LOG(options_.info_log, "[%s] start Init()", dbname_.c_str());
  Status s;
  if (options_.use_file_lock) {
    s = env_->LockFile(LockFileName(dbname_), &db_lock_);
    if (!s.ok()) {
      LEVELDB_LOG(options_.info_log, "[%s] Get db lock fail", dbname_.c_str());
      return s;
    }
  }
  ...
  for (std::set<uint32_t>::iterator it = options_.exist_lg_list->begin();
    ...
    uint32_t i = *it;
    DBImpl* impl = new DBImpl(InitOptionsLG(options_, i), dbname_ + "/" + Uint64ToString(i));
    ...
    // recover SST
    LEVELDB_LOG(options_.info_log, "[%s] start Recover lg%d, last_seq= %lu", dbname_.c_str(), i,
                impl->GetLastSequence());
    s = impl->Recover(lg_edits[i]);
    LEVELDB_LOG(options_.info_log, "[%s] end Recover lg%d, last_seq= %lu", dbname_.c_str(), i,
                impl->GetLastSequence());
    if (s.ok()) {
      uint64_t last_seq = impl->GetLastSequence();

      LEVELDB_LOG(options_.info_log, "[%s] Recover lg %d last_log_seq= %lu", dbname_.c_str(), i,
                  last_seq);
      ...
      LEVELDB_LOG(options_.info_log, "[%s] fail to recover lg %d", dbname_.c_str(), i);
      break;
    }
  }
  if (!s.ok()) {
    LEVELDB_LOG(options_.info_log, "[%s] fail to recover table.", dbname_.c_str());
    for (uint32_t i = 0; i != lg_list_.size(); ++i) {
      delete lg_list_[i];
    }
    lg_list_.clear();
  ...
  LEVELDB_LOG(options_.info_log, "[%s] start GatherLogFile", dbname_.c_str());
  // recover log files
  std::vector<uint64_t> logfiles;
  ...
  if (s.ok()) {
    for (uint32_t i = 0; i < logfiles.size(); ++i) {
      ...
      if (!s.ok()) {
        LEVELDB_LOG(options_.info_log, "[%s] Fail to RecoverLogFile %ld", dbname_.c_str(),
                    logfiles[i]);
      }
    }
  } else {
    LEVELDB_LOG(options_.info_log, "[%s] Fail to GatherLogFile", dbname_.c_str());
  }

  LEVELDB_LOG(options_.info_log, "[%s] start RecoverLogToLevel0Table", dbname_.c_str());
  ...
  if (s.ok()) {
    LEVELDB_LOG(options_.info_log, "[%s] start DeleteLogFile", dbname_.c_str());
    s = DeleteLogFile(logfiles);
  }
  ...
  if (s.ok() && !options_.disable_wal) {
    ...
    if (s.ok()) {
      // LEVELDB_LOG(options_.info_log, "[%s] open logfile %s",
      //    dbname_.c_str(), log_file_name.c_str());
      log_ = new log::AsyncWriter(logfile_, options_.log_async_mode);
    } else {
      LEVELDB_LOG(options_.info_log, "[%s] fail to open logfile %s", dbname_.c_str(),
                  log_file_name.c_str());
    }
  }
  ...
  if (s.ok()) {
    state_ = kOpened;
    LEVELDB_LOG(options_.info_log, "[%s] custom compact strategy: %s, flush trigger %lu",
                dbname_.c_str(), options_.compact_strategy_factory->Name(),
                options_.flush_triggered_log_num);

    commit_snapshot_ = last_sequence_;
    LEVELDB_LOG(options_.info_log, "[%s] Init() done, last_seq=%llu", dbname_.c_str(),
...
Status DBTable::Put(const WriteOptions& options, const Slice& key, const Slice& value) {
...
Status DBTable::Write(const WriteOptions& options, WriteBatch* my_batch) {
    ...
    for (;; wait_sec <<= 1) {
      ...
      if (s.IsTimeOut()) {
        LEVELDB_LOG(options_.info_log,
                    "[%s] AddRecord time out, current log size: %lu, "
      ...
      if (!s.ok()) {
        ...
        s = log_->WaitDone(wait_sec);
        if (s.IsTimeOut()) {
          LEVELDB_LOG(options_.info_log, "[%s] Sync time out %lu", dbname_.c_str(),
                      current_log_size_);
          int ret = SwitchLog(true);
          if (ret == 0) {
            continue;
  ...
  if (s.ok()) {
    ...
    for (uint32_t i = 0; i < lg_updates.size(); ++i) {
      ...
      if (!lg_s.ok()) {
        // 这种情况下内存处于不一致状态
        LEVELDB_LOG(options_.info_log, "[%s] [Fatal] Write to lg%u fail", dbname_.c_str(), i);
        s = lg_s;
        break;
      }
    }
...
Status DBTable::Get(const ReadOptions& options, const Slice& key, std::string* value) {
  uint32_t lg_id = 0;
  Slice real_key = key;
...
Iterator* DBTable::NewIterator(const ReadOptions& options) {
  ...
  for (; it != options_.exist_lg_list->end(); ++it) {
    if (options.target_lgs) {
      std::set<uint32_t>::const_iterator found_it = options.target_lgs->find(*it);
...
void DBTable::GetApproximateSizes(const Range* range, int n, uint64_t* sizes) {
  for (int j = 0; j < n; ++j) {
    sizes[j] = 0;
  }
...
void DBTable::GetApproximateSizes(uint64_t* size, std::vector<uint64_t>* lgsize,
  ...
  for (; it != options_.exist_lg_list->end(); ++it) {
    uint32_t i = *it;
    uint64_t size_tmp;
    uint64_t mem_table_size_tmp = 0;
    lg_list_[i]->GetApproximateSizes(&size_tmp, nullptr, &mem_table_size_tmp);
    if (size) {
      *size += size_tmp;
    }
    if (mem_table_size) {
      *mem_table_size += mem_table_size_tmp;
    }
    if (lgsize) {
...
Status DBTable::GatherLogFile(uint64_t begin_num, std::vector<uint64_t>* logfiles) {
  std::vector<std::string> files;
  Status s = env_->GetChildren(dbname_, &files);
  if (!s.ok()) {
    LEVELDB_LOG(options_.info_log, "[%s] GatherLogFile fail", dbname_.c_str());
    return s;
  }
  uint64_t number = 0;
  ...
  for (uint32_t i = 0; i < files.size(); ++i) {
    type = kUnknown;
    number = 0;
    if (ParseFileName(files[i], &number, &type) && type == kLogFile && number >= begin_num) {
  ...
  if ((last_number > 0 && first_log_num > begin_num)    // case 1
    ...
    LEVELDB_LOG(options_.info_log, "[%s] add log file #%lu", dbname_.c_str(), last_number);
  }
  LEVELDB_LOG(options_.info_log,
              "[%s] begin_seq= %lu, first log num= %lu, last num=%lu, log count=%lu\n",
...
Status DBTable::RecoverLogFile(uint64_t log_number, uint64_t recover_limit,
  ...
  struct LogReporter : public log::Reader::Reporter {
    ...
    virtual void Corruption(size_t bytes, const Status& s) {
      LEVELDB_LOG(info_log, "%s%s: dropping %d bytes; %s",
                  (this->status == NULL ? "(ignoring error) " : ""), fname, static_cast<int>(bytes),
                  s.ToString().c_str());
  ...
  LEVELDB_LOG(options_.info_log, "[%s] Recovering log #%lx, sequence limit %lu", dbname_.c_str(),
              log_number, recover_limit);
  ...
  while (reader.ReadRecord(&record, &scratch) && status.ok()) {
    ...
    // LEVELDB_LOG(options_.info_log, "[%s] batch_seq= %lu, last_seq= %lu,
    // count=%d",
    //    dbname_.c_str(), batch_seq, last_sequence_,
    //    WriteBatchInternal::Count(&batch));
    if (last_seq >= recover_limit) {
      LEVELDB_LOG(options_.info_log, "[%s] exceed limit %lu, ignore %lu ~ %lu", dbname_.c_str(),
                  recover_limit, first_seq, last_seq);
      continue;
    }
    ...
    if (status.ok()) {
      ...
      for (uint32_t i = 0; i < lg_updates.size(); ++i) {
        ...
        // LEVELDB_LOG(options_.info_log, "[%s] recover log batch first= %lu,
        // last= %lu\n",
        //     dbname_.c_str(), first, last);

        Status lg_s = lg_list_[i]->RecoverInsertMem(lg_updates[i], (*edit_list)[i]);
        if (!lg_s.ok()) {
          LEVELDB_LOG(options_.info_log, "[%s] recover log fail batch first= %lu, last= %lu\n",
                      dbname_.c_str(), first, last);
          status = lg_s;
        }
      }
...
Status DBTable::DeleteLogFile(const std::vector<uint64_t>& log_numbers) {
  ...
  for (uint32_t i = 0; i < log_numbers.size() && s.ok(); ++i) {
    ...
    LEVELDB_LOG(options_.info_log, "[%s] Delete type=%s #%llu", dbname_.c_str(),
                FileTypeToString(kLogFile), static_cast<unsigned long long>(log_number));
    std::string fname = LogHexFileName(dbname_, log_number);
    ...
    if (!s.ok()) {
      LEVELDB_LOG(options_.info_log, "[%s] fail to delete logfile %llu: %s", dbname_.c_str(),
                  static_cast<unsigned long long>(log_number), s.ToString().data());
    }
  }
  return s;
}
...
void DBTable::DeleteObsoleteFiles(uint64_t seq_no) {
  ...
  LEVELDB_LOG(options_.info_log, "[%s] delete obsolete log: %u, keep: %u, [seq < %llu]",
              dbname_.c_str(), static_cast<uint32_t>(delete_log_num),
              static_cast<uint32_t>(keep_log_num), static_cast<unsigned long long>(seq_no));
}
...
void DBTable::ArchiveFile(const std::string& fname) {
  ...
  LEVELDB_LOG(options_.info_log, "[%s] Archiving %s: %s\n", dbname_.c_str(), fname.c_str(),
              s.ToString().c_str());
}
...
bool DBTable::FindSplitKey(double ratio, std::string* split_key) {
  ...
  std::map<uint64_t, DBImpl*>::reverse_iterator biggest_it = size_of_lg.rbegin();
...
void DBTable::AddInheritedLiveFiles(std::vector<std::set<uint64_t> >* live) {
  ...
  // LEVELDB_LOG(options_.info_log, "[%s] finish collect inherited sst fils",
  //    dbname_.c_str());
}

bool DBTable::IsShutdown1Finished() const { return shutdown1_finished_.Acquire_Load() != NULL; }
...
int DBTable::SwitchLog(bool blocked_switch) {
  {
    ...
    if (fatal_error_.IsIOPermissionDenied() || IsShutdown1Finished()) {
      if (IsShutdown1Finished()) {
        fatal_error_ =
            Status::ShutdownInProgress(dbname_ + ": fail to switch log on waiting shutdown2");
      }
      LEVELDB_LOG(options_.info_log, "[%s] can not switch log becasue %s", dbname_.c_str(),
                  fatal_error_.ToString().c_str());
      return 2;
    }
  }
  if (!blocked_switch || log::AsyncWriter::BlockLogNum() < options_.max_block_log_number) {
    ...
    if (s.ok()) {
      ...
      if (blocked_switch) {
        ...
        LEVELDB_LOG(options_.info_log, "[%s] SwitchLog", dbname_.c_str());
      }
      return 0;  // success
      ...
      LEVELDB_LOG(options_.info_log, "[%s] fail to open logfile %s. SwitchLog failed",
                  dbname_.c_str(), log_file_name.c_str());
      if (!blocked_switch) {
...
void DBTable::ScheduleGarbageClean(double score) {
  ...
  if (bg_schedule_gc_ && score <= bg_schedule_gc_score_) {
    ...
    LEVELDB_LOG(options_.info_log, "[%s] ReSchedule Garbage clean[%ld] score= %.2f",
    ...
    LEVELDB_LOG(options_.info_log, "[%s] Schedule Garbage clean[%ld] score= %.2f", dbname_.c_str(),
                bg_schedule_gc_id_, score);
  }
}
...
void DBTable::GarbageClean() {
  ...
  if (force_clean_log_seq_ > min_last_seq) {
    LEVELDB_LOG(options_.info_log, "[%s] force_clean_log_seq_= %lu, min_last_seq= %lu",
  ...
  if (found && min_last_seq > 0) {
    LEVELDB_LOG(options_.info_log, "[%s] delete obsolete file, seq_no below: %lu", dbname_.c_str(),
                min_last_seq);
