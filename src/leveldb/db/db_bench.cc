...
static int FLAGS_write_buffer_size = 32 * 1024 * 1024;
...
// evn
static const char* FLAGS_env = NULL;

// disable WAL
static bool FLAGS_disable_wal = false;

// compress
static int FLAGS_compress = 0;

// block size
static int FLAGS_block_size = 4096;
...
struct ThreadState {
  ...
  ThreadState(int index) : tid(index), rand(1000 + index), shared(NULL) {}
};
...
class Benchmark {
 ...
 public:
  ...
  void Run() {
    ...
    while (benchmarks != NULL) {
      ...
      } else if (name == Slice("bmzcomp")) {
        method = &Benchmark::BmzCompress;
      } else if (name == Slice("bmzuncomp")) {
        method = &Benchmark::BmzUncompress;
#endif
 ...
 private:
  ...
  void LZ4Uncompress(ThreadState* thread) {
    ...
    int64_t bytes = 0;
    char* uncompressed = new char[input.size()];
    size_t max_buffer_size = input.size() * 3;
    while (ok && bytes < 1024 * 1048576) {  // Compress 1G
      ok = port::Lz4_Uncompress(compressed.data(), compressed.size(), uncompressed,
                                &max_buffer_size);
      bytes += input.size();
      thread->stats.FinishedSingleOp();
    }
  ...
  void BmzCompress(ThreadState* thread) {
    RandomGenerator gen;
    Slice input = gen.Generate(Options().block_size);
    int64_t bytes = 0;
    int64_t produced = 0;
    ...
    std::string compressed;
    while (ok && bytes < 1024 * 1048576) {  // Compress 1G
      ok = port::Bmz_Compress(input.data(), input.size(), &compressed);
      produced += compressed.size();
      bytes += input.size();
      thread->stats.FinishedSingleOp();
    }
    ...
    if (!ok) {
      thread->stats.AddMessage("(BMZ failure)");
    } else {
      char buf[100];
      snprintf(buf, sizeof(buf), "(output: %.1f%%)", (produced * 100.0) / bytes);
      thread->stats.AddMessage(buf);
  ...
  void BmzUncompress(ThreadState* thread) {
    RandomGenerator gen;
    Slice input = gen.Generate(Options().block_size);
    std::string compressed;
    bool ok = port::Bmz_Compress(input.data(), input.size(), &compressed);
    int64_t bytes = 0;
    char* uncompressed = new char[input.size()];
    size_t max_buffer_size = input.size() * 3;
    while (ok && bytes < 1024 * 1048576) {  // Compress 1G
      ok = port::Bmz_Uncompress(compressed.data(), compressed.size(), uncompressed,
                                &max_buffer_size);
      bytes += input.size();
      thread->stats.FinishedSingleOp();
    }
  ...
  CompressionType NumToCompressionType(int n) {
    if (n == 1) {
      return kSnappyCompression;
    } else if (n == 2) {
      return kBmzCompression;
    } else if (n == 3) {
  ...
  void Open() {
    ...
    options.compression = NumToCompressionType(FLAGS_compress);
    Status log_s = Env::Default()->NewLogger("./ldblog", LogOption::LogOptionBuilder().Build(),
                                             &options.info_log);
    if (FLAGS_env == NULL) {
      // do nothing
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    } else if (sscanf(argv[i], "--disable_wal=%d%c", &n, &junk) == 1 && (n == 0 || n == 1)) {
      FLAGS_disable_wal = n;
    } else if (sscanf(argv[i], "--compress=%d%c", &n, &junk) == 1 &&
               (n == 0 || n == 1 || n == 2 || n == 3)) {
      FLAGS_compress = n;
