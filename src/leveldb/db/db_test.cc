 ...
 public:
  ...
  Status NewWritableFile(const std::string& f, WritableFile** r, const EnvOptions&) {
    class InitLoadLockFile : public WritableFile {
     ...
     public:
      InitLoadLockFile(SpecialEnv* env, WritableFile* base) : env_(env), base_(base) {}
    ...
    class SSTableFile : public WritableFile {
     ...
     public:
      SSTableFile(SpecialEnv* env, WritableFile* base) : env_(env), base_(base) {}
    ...
    class ManifestFile : public WritableFile {
     ...
     public:
      ...
      Status Sync() {
        ...
        if (env_->manifest_sync_error_.Acquire_Load() != NULL && env_->sync_retry_c_.Read() < 10) {
...
class DBTest {
 ...
 public:
  ...
  Options SplitOptions() {
    ...
    options.dump_mem_on_shutdown = false;
    Logger* logger;
    Env::Default()->NewLogger("/tmp/db_test_split.log", LogOption::LogOptionBuilder().Build(),
                              &logger);
    Env::Default()->SetLogger(logger);
    options.info_log = logger;
    switch (option_config_) {
  ...
  DBTable* dbfull() { return reinterpret_cast<DBTable*>(db_); }
  ...
  while (t->state->stop.Acquire_Load() == NULL) {
    ...
    if (rnd.OneIn(2)) {
      ...
      if (s.IsNotFound()) {
        ...
        ASSERT_LE(c, reinterpret_cast<uintptr_t>(t->state->counter[w].Acquire_Load()));
      }
    }
    counter++;
  }
...
class ModelDB : public DB {
 public:
  ...
  virtual const uint64_t Rollback(uint64_t snapshot_seq,
                                  uint64_t rollback_point = kMaxSequenceNumber) {
    // TODO
    return 0;
  }
  virtual bool BusyWrite() { return false; }
  ...
  virtual void GetApproximateSizes(uint64_t* size, std::vector<uint64_t>* lgsize = NULL,
                                   uint64_t* mem_size = NULL) {}

  virtual void CompactRange(const Slice* start, const Slice* end, int lg_no) {}

  virtual bool FindSplitKey(double ratio, std::string* split_key) { return false; }

  virtual bool FindKeyRange(std::string* smallest_key, std::string* largest_key) { return false; }

  virtual void GetCurrentLevelSize(std::vector<int64_t>* result) {}

  virtual uint64_t GetScopeSize(const std::string& start_key, const std::string& end_key,
                                std::vector<uint64_t>* lgsize = NULL) {
    return 0;
  }

  virtual bool MinorCompact() { return false; }
...
static bool CompareIterators(int step, DB* model, DB* db, uint64_t model_snap, uint64_t db_snap) {
  ReadOptions options;
  ...
  for (miter->SeekToFirst(), dbiter->SeekToFirst(); ok && miter->Valid() && dbiter->Valid();
    ...
    if (miter->key().compare(dbiter->key()) != 0) {
      fprintf(stderr, "step %d: Key mismatch: '%s' vs. '%s' [count: %d]\n", step,
              EscapeString(miter->key()).c_str(), EscapeString(dbiter->key()).c_str(), count);
      ok = false;
      break;
    }
...
TEST(DBTest, Randomized) {
  ...
  do {
    ...
    for (int step = 0; step < N; step++) {
      ...
      if ((step % 100) == 0) {
        ASSERT_TRUE(CompareIterators(step, &model, db_, leveldb::kMaxSequenceNumber,
