...
class HashTable {
 public:
  explicit HashTable(const size_t capacity = 1024 * 1024, const float load_factor = 2.0,
    ...
    // initialize locks
    locks_.reset(new RWMutex[nlocks_]);
    mlock(locks_.get(), nlocks_ * sizeof(RWMutex));
 ...
 protected:
  ...
  std::unique_ptr<RWMutex[]> locks_;   // Granular locks
};
}  // namespace leveldb