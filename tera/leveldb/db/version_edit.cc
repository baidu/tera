...
Status VersionEdit::DecodeFilterFrom(const Slice& src,
      ...
      case kComparator:
        if (GetLengthPrefixedSlice(&input, &str)) {
            if (comparator_.empty()) {
                comparator_ = str.ToString();
            } else if (comparator_ != str.ToString()) {
                msg = "unmatch comparator";
            }
          has_comparator_ = true;
        } else {
          msg = "comparator name";
